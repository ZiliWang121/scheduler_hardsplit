...
class Env():
	...
	def __init__(self,fd,time,k,alpha,b,c,max_flows):
		...
		self.path_mask = [[0] for _ in range(self.max_num_flows)]	#path mask for ordering from user space to kernel space
	...
	def reward(self):
		...
		if rewards!=0:
			rewards = rewards - (1/rewards)*self.alpha* sum(np.array(self.rtt)[:,self.k-1]*(np.array(self.tp)[:,self.k-1]))
			rewards = rewards - self.b * (1/sum(np.array(self.tp)[:,self.k-1]))*sum(np.array(self.rr)[:,self.k-1]*(np.array(self.tp)[:,self.k-1]))
		else:
			rewards = 0
		return rewards
